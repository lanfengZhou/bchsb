...
router.post('/add',function(req,res){
	...
	query('insert into crops(name,description,insertTime) values (\"'+name+'\",\"'+description+'\",\"'+date+'\")',function(err,vals,fileds){
		if(err){
			...
			var cropsDirPath = globaldir + name;
...
router.post('/delete',function(req,res){
	...
	query('delete from crops where id='+id,function(err,vals,fileds){
		if(err){
			...
			var cropsDirPath = globaldir + name;
			rmdirSync(cropsDirPath,function(){
 ...
 * 获取农作物
 * @param  {[type]} req                  [description]
 * @param  {[type]} res){	query('select id,name       from crops',function(err,vals,fileds){		if(err){			console.log(err);			res.json({'success':true,'result':'failed'});		}else{			res.json({'success':true,'crops':vals});		}	});} [description]
 * @return {[type]}                      [description]
 */
